...
def PID_r(target, kp, ki, kd, speed, colour=None):
    ...
    while not color_sensor.color(port.C) == colour:
        ...
        # kp: increase if robot corrects too little / undershoots line, vice versa
        proportional = error * kp
        # ki: usually keep to 0 as it affects robot stability in the long term
        integral += error * ki
        # kd: increase to help stabilise the robot, decrease if robot very jerky (small jerks)
        derivative = (error - last_error) * kd

        sum = round(proportional + integral +
                    derivative)                # movement
        # robot should automatically correct
        motor_pair.move_tank(motor_pair.PAIR_1, speed + sum, speed - sum)
...
def PID_l(target, kp, ki, kd, speed, colour=None):
    ...
    # if other light sensor senses a different colour (eg. when the robot reaches the desired location)
    while not color_sensor.color(port.D) == colour:
...
def gyro_straight(distance, multiplier, speed):
    ...
    # print(cm(distance))
    target = 0
    avg_dist = (abs(motor.relative_position(port.A)) +
                abs(motor.relative_position(port.B))) / 2
    # print(target)
    ...
    while avg_dist < cm(distance):
        ...
        motor_pair.move_tank(motor_pair.PAIR_1, round(
            speed - correct), round(speed + correct))
        # print(angle, error)
...
def main():
    print("start")
    # defining our large/driving motors
